...
export default function PlayerLoginPage() {
  ...
  const { user, role, signInWithEmail, signInPlayerWithGoogle, loading, error, clearError } =
    useAuth();
  ...
  // If already logged in as player, route to /player
  useEffect(() => {
    if (!loading && user) {
    ...
    <div className="flex min-h-screen flex-col justify-center px-4 py-12 sm:px-6 lg:px-8 bg-qc-black">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-1 text-center text-xs text-qc-muted uppercase tracking-widest">
          Find Your Game & Book Nearby Venues
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="border border-white/10 bg-qc-panel p-6 sm:p-8 shadow-2xl">
          ...
          <div className="mt-6 pt-6 border-t border-white/8 text-center">
            <p className="text-xs text-qc-muted">
              New to QuickCourt?{" "}
              <Link href="/player/signup" className="text-qc-lime hover:underline font-semibold">
        ...
        <div className="mt-6 text-center">
          ...
          >
            ...
            Are you a sports facility owner? Sign in to Owner Portal →
          </Link>
        </div>
      </div>
    </div>
